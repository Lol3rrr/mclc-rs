...
fn place_edge(space: &mut Space<SpaceCell>, edge: &graph::normalized::Edge, nodes: &[PlacedNode]) {
    ...
    let src_node = nodes
        .iter()
        .find(|PlacedNode(pos, id, d)| *id == src_id)
        .unwrap();
    let src_pos = match &src_node.2 {
        PlacedNodeData::Input { .. } => src_node.0.clone(),
        PlacedNodeData::Output { .. } => src_node.0.clone(),
        PlacedNodeData::Variable { .. } => src_node.0.clone(),
        PlacedNodeData::Splitter { ports, .. } => {
            ports.get(edge.src_port as usize).unwrap().clone()
        }
        PlacedNodeData::Entity { out_ports, .. } => {
            out_ports.get(edge.src_port as usize).unwrap().clone()
        }
    };
    ...
    let dest_node = nodes
        .iter()
        .find(|PlacedNode(pos, id, d)| *id == dest_id)
        .unwrap();
    let dest_pos = match &dest_node.2 {
        PlacedNodeData::Input { .. } => dest_node.0.clone(),
        PlacedNodeData::Output { .. } => dest_node.0.clone(),
        PlacedNodeData::Variable { .. } => dest_node.0.clone(),
        PlacedNodeData::Splitter { input, .. } => input.clone(),
        PlacedNodeData::Entity { in_ports, .. } => {
            in_ports.get(edge.dest_port as usize).unwrap().clone()
        }
    };
